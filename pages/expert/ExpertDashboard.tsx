...
const ExpertDashboard = () => {
  const { user, requests, assignRequest, updateRequestStatus } = useAppContext();
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      {isVetting && (
          <div className="bg-orange-50 border border-orange-200 rounded-xl p-4 flex items-start gap-3">
              ...
              <div>
                  <h3 className="font-bold text-orange-900">Account Under Review</h3>
                  <p className="text-sm text-orange-700 mt-1">
                      Welcome to Finume! Your profile is currently being vetted by our administration team. 
                      You can explore the dashboard, but you cannot accept client requests until your status is <strong>Active</strong>.
                  </p>
              </div>
          </div>
      )}
      ...
      <div className="flex flex-col md:flex-row justify-between items-end gap-4 bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
        <div>
            ...
            <p className="text-gray-500 mt-1">You have <span className="font-bold text-blue-600">{activeTasks.length} active tasks</span> requiring attention today.</p>
        </div>
        <div className="flex items-center gap-4">
            <div className="flex items-center gap-2 bg-gray-50 px-3 py-2 rounded-lg border border-gray-100">
                <span className={`w-3 h-3 rounded-full ${isOnline ? 'bg-green-500' : 'bg-gray-400'}`}></span>
                <span className="text-sm font-medium text-gray-700">{isOnline ? 'Available for Work' : 'Away'}</span>
                <button 
            ...
            <Button onClick={() => navigate('/expert/earnings')} className="shadow-lg shadow-green-100 bg-emerald-600 hover:bg-emerald-700">
                <DollarSign size={18} /> View Wallet
            </Button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <Card className="border-l-4 border-l-emerald-500 hover:shadow-md transition-all">
                <div className="flex justify-between items-start">
                    <div>
                        ...
                        <h3 className="text-2xl font-extrabold text-gray-900 mt-1">{totalEarnings.toLocaleString()} <span className="text-sm font-medium text-gray-400">SAR</span></h3>
                    </div>
        ...
        <div onClick={() => navigate('/expert/profile')} className="cursor-pointer group">
            <Card className="border-l-4 border-l-yellow-500 hover:shadow-md transition-all">
                <div className="flex justify-between items-start">
                    <div>
                        <p className="text-xs font-bold text-gray-400 uppercase tracking-wider">Client Rating</p>
        ...
        <div className="group">
            <Card className="border-l-4 border-l-purple-500 hover:shadow-md transition-all">
                <div className="flex justify-between items-start">
                    <div>
                        <p className="text-xs font-bold text-gray-400 uppercase tracking-wider">Job Success</p>
             ...
             <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                 {marketplaceRequests.length > 0 ? (
                        ...
                        {marketplaceRequests.slice(0, 5).map(req => (
                            <div key={req.id} className={`p-5 hover:bg-gray-50 transition-colors flex flex-col sm:flex-row justify-between gap-4 ${isVetting ? 'opacity-75 grayscale-[0.5]' : ''}`}>
                                ...
                                <div className="flex items-center justify-between sm:justify-end gap-4 min-w-[160px]">
                                    <span className="font-bold text-gray-900">{req.amount.toLocaleString()} SAR</span>
                                    <Button 
                                        size="sm" 
                                        variant="outline" 
                    ...
                    <div className="text-center py-12">
                        ...
                        <p className="text-gray-500">No new jobs available in the marketplace right now.</p>
                    </div>
                 )}
                 {marketplaceRequests.length > 0 && (
                     <div className="p-3 bg-gray-50 border-t border-gray-100 text-center">
                         <button className="text-sm font-medium text-primary-600 hover:text-primary-700">View All Jobs</button>
                     </div>
                 )}
             </div>
           </div>
        ...
        <div className="space-y-6 sticky top-24 self-start">
            ...
            <Card className="border border-gray-100 shadow-sm">
                <div className="flex justify-between items-center mb-4">
                    ...
                    <div className="p-1 bg-gray-100 rounded text-xs text-gray-500 px-2">This Week</div>
                </div>
                <div className="h-[180px] w-full relative">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center flex-col pointer-events-none">
                        ...
                        <span className="text-[10px] text-gray-400 font-bold uppercase tracking-wider">Tasks</span>
                    </div>
                </div>
            ...
            {/* Quick Actions */}
            <Card className="border border-gray-100 shadow-sm">
                <h3 className="font-bold text-gray-800 mb-4 text-sm uppercase tracking-wide text-gray-500">Quick Actions</h3>
                <div className="space-y-3">
                    <button className="w-full flex items-center justify-between p-3 bg-white border border-gray-200 rounded-xl hover:border-blue-400 hover:shadow-md transition-all group">
                        <div className="flex items-center gap-3">
                            ...
                            <span className="text-sm font-medium text-gray-700">Update Portfolio</span>
                        </div>
                    ...
                    >
                        <div className="flex items-center gap-3">
                            ...
                            <span className="text-sm font-medium text-gray-700">Boost Profile</span>
                        </div>
            ...
            <div className="relative overflow-hidden bg-gradient-to-br from-indigo-900 to-blue-800 rounded-2xl p-6 text-white text-center shadow-lg group cursor-pointer hover:scale-[1.02] transition-transform">
                ...
                <div className="relative z-10">
                    ...
                    <p className="text-indigo-200 text-xs mb-4 px-2">Contact expert support for priority assistance.</p>
