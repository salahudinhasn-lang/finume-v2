  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
       ...
       <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
         ...
         <Card className={`relative overflow-hidden border transition-all p-5 shadow-sm flex flex-col justify-between ${
           ...
           {(selectedTxIds.size > 0 || availableForPayout > 0) && (
                <button 
                    ...
                    className={`w-full mt-3 text-xs font-bold py-2 rounded-lg shadow-sm flex items-center justify-center gap-2 transition-all active:scale-95 ${
                        isProcessing 
             ...
             <table className="w-full text-sm text-left">
                ...
                <tbody className="divide-y divide-gray-100">
                   {filteredLedger.map(req => {
                      ...
                      return (
                        <tr key={req.id} className={`hover:bg-gray-50 transition-colors ${status === 'UNSETTLED' ? 'bg-orange-50/10' : ''} ${isSelected ? 'bg-amber-50' : ''}`}>
                           ...
                           <td className="px-6 py-4 font-mono text-gray-500 text-xs">INV-{req.id.split('-')[1]}</td>
                   ...
                   {filteredLedger.length === 0 && (
                      <tr>
                         <td colSpan={9} className="text-center py-12 text-gray-500">No financial records found matching your filters.</td>
                      </tr>
                   )}
                </tbody>
             </table>
