                        ...
                        {payments.slice(0, 5).map(pay => (
                            <div key={pay.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg border border-gray-100">
                                ...
                                <div className="text-right">
                                    ...
                                    <p className="text-[10px] text-green-600 uppercase font-bold">Paid</p>
                                </div>
                            </div>
                        ))}
                    </div>
          ...
          <table className="w-full text-sm text-left">
            <thead className="bg-white text-gray-500 font-semibold border-b border-gray-200">
              <tr>
                ...
                <th className="px-6 py-4 text-right">VAT (15%)</th>
              ...
              {payments.map(pay => (
                <tr key={pay.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-6 py-4 font-mono text-gray-600 font-medium text-xs">
                    {pay.id}
                    {pay.type === 'CREDIT NOTE' && <span className="block text-[10px] text-red-500">Credit Note</span>}
                  </td>
                  <td className="px-6 py-4">
                      <span className="font-medium text-gray-800 block">{pay.description}</span>
                      <span className="text-xs text-gray-400">Proof of payment for Request #{pay.requestId}</span>
                  </td>
           ...
           <div className="bg-white rounded-xl w-full max-w-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] print:max-h-full print:shadow-none print:rounded-none">
               ...
               <div className="p-8 md:p-12 overflow-y-auto bg-white flex-1" id="invoice-content">
                   ...
                   <div className="flex justify-between items-start border-b border-gray-200 pb-8 mb-8">
                       ...
                       <div className="text-right">
                           <h2 className={`text-2xl font-bold mb-1 ${selectedInvoice.type === 'CREDIT NOTE' ? 'text-red-600' : 'text-gray-900'}`}>
                               {selectedInvoice.type}
                           </h2>
                           <p className="text-gray-500 font-mono mb-4">{selectedInvoice.id}</p>
                           <div className={`inline-block px-3 py-1 rounded font-bold text-sm border ${
                   ...
                   <div className="grid grid-cols-2 gap-8 mb-8">
                       <div>
                           <p className="text-sm font-bold text-gray-400 uppercase mb-2">
                               {selectedInvoice.type === 'CREDIT NOTE' ? 'Credit To' : 'Bill To'}
                           </p>
                           ...
                           <tr className="border-b border-gray-100">
                               <td className="py-4 text-gray-800">
                                   ...
                                   <div className="text-xs text-gray-500 mt-1">
                                       {selectedInvoice.type === 'CREDIT NOTE' 
                                         ? 'Refund for cancellation of services.' 
                                         : 'Professional financial services provided by expert.'
                                       }
                                   </div>
                               </td>
                   ...
                   <div className="border-t border-gray-200 pt-8 text-center text-sm text-gray-500">
                       ...
                       <p className="text-xs text-gray-400">This is a computer-generated document and does not require a physical signature.</p>
                   </div>
               </div>
           </div>
        </div>
