...
const BrowseExperts = () => {
  ...
  // Filter for ACTIVE and PREMIUM experts only
  const activeExperts = experts.filter(e => e.status === 'ACTIVE' && e.isPremium);
  ...
  const ExpertCard: React.FC<{ expert: Expert, isFeatured?: boolean }> = ({ expert, isFeatured }) => (
    <Card className={`flex flex-col h-full hover:shadow-xl transition-all duration-300 relative group overflow-hidden ${expert.isPremium ? 'border-amber-200' : 'border-gray-100'}`}>
        {expert.isPremium && (
        ...
        {/* Background Header */}
        <div className={`h-20 w-full ${expert.isPremium ? 'bg-gradient-to-r from-amber-50 to-orange-50' : 'bg-gradient-to-r from-gray-50 to-gray-100'}`}></div>

        <div className="px-6 relative flex-1 flex flex-col">
            <div className="flex justify-between items-end -mt-10 mb-4">
                <div className="relative">
                    <img src={expert.avatarUrl} alt={expert.name} className={`w-20 h-20 rounded-full bg-white object-cover border-4 ${expert.isPremium ? 'border-amber-100' : 'border-white'} shadow-md`} />
            ...
            <div className="mb-4">
                <div className="flex items-center gap-2">
                    ...
                    {expert.isFeatured && <Award size={16} className="text-blue-500" />}
                </div>
            ...
            <div className="pt-4 border-t border-gray-100 flex items-center justify-between mt-auto mb-6">
                ...
                <Button onClick={() => handleHire(expert.id)} size="sm" className={expert.isPremium ? 'bg-amber-500 hover:bg-amber-600 border-none shadow-lg shadow-amber-200' : 'shadow-md'}>Hire Now</Button>
            </div>
        </div>
    </Card>
  );
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col gap-6">
        <div className="flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <div className="relative w-full md:w-96">
                ...
                <input 
                    ...
                    className="w-full pl-10 pr-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-500 transition-all shadow-sm"
                />
            </div>
        </div>
        ...
        <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
          <button 
            ...
            className={`px-5 py-2.5 rounded-lg text-sm font-bold whitespace-nowrap transition-all ${filter === 'All' ? 'bg-gray-800 text-white shadow-md' : 'bg-white text-gray-600 hover:bg-gray-50 border border-gray-200'}`}
          >
            All Experts
          </button>
          {allSpecializations.map(spec => (
            <button 
              ...
              className={`px-5 py-2.5 rounded-lg text-sm font-medium whitespace-nowrap transition-all ${filter === spec ? 'bg-gray-800 text-white shadow-md' : 'bg-white text-gray-600 hover:bg-gray-50 border border-gray-200'}`}
            >
              {spec}
            </button>
          ))}
      ...
      {filteredExperts.length === 0 && (
          <div className="text-center py-20">
              ...
              <h3 className="text-lg font-bold text-gray-600">No premium experts found</h3>
