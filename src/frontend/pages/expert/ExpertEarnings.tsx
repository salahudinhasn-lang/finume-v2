...
const ExpertEarnings = () => {
    ...
    // Helper to calculate expert share based on Plan/Service config
    ...
    // Expert gets dynamic share
    const lifetimeEarnings = myCompletedRequests.reduce((acc, r) => acc + calculateExpertShare(r), 0);

    // 2. Unsettled Requests (Available for Payout)
    const unsettledRequests = myCompletedRequests.filter(r => !r.payoutId);
    const totalUnsettledBalance = unsettledRequests.reduce((acc, r) => acc + calculateExpertShare(r), 0);
    ...
    const pendingBalance = pendingRequests.reduce((acc, r) => acc + calculateExpertShare(r), 0);
    ...
    const totalWithdrawn = settledRequests.reduce((acc, r) => acc + calculateExpertShare(r), 0);
    ...
    const pendingEarnings = pipelineRequests.reduce((acc, curr) => acc + calculateExpertShare(curr), 0);
                    ...
                    <Card className="h-full flex flex-col">
                        ...
                        <div className="space-y-0 overflow-y-auto flex-1 custom-scrollbar max-h-[400px]">
                            {myCompletedRequests.length > 0 ? myCompletedRequests.map(req => {
                                ...
                                const amount = calculateExpertShare(req);
                                ...
                                return (
                                    <div
                                        ...
                                        className={`p-4 border-b border-gray-100 last:border-0 hover:bg-gray-50 transition-colors cursor-pointer ${status === 'UNSETTLED' && isSelected ? 'bg-blue-50/50' : ''
                                            }`}
                                    >
                                        ...
                                        <div className="flex justify-between items-center text-xs pl-6">
                                            ...
                                            <span className={`px-2 py-0.5 rounded font-bold uppercase text-[10px] ${status === 'SETTLED' ? 'bg-green-100 text-green-700' :
