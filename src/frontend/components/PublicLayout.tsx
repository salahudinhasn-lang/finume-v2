...
const PublicLayout = () => {
  ...
  const getVisibility = (key: string) => {
    if (!settings?.pageVisibility) return true; // Default to visible if no settings
    try {
      const vis = JSON.parse(settings.pageVisibility);
      return vis[key.toLowerCase()]?.public !== false;
    } catch (e) {
      return true;
    }
  };
      ...
      <footer className="bg-[#0B0F19] text-slate-300 border-t border-slate-800 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto pt-20 pb-12 px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 lg:gap-12">
            ...
            {/* Column 1 - Static Platform Links */}
            <div>
              <h3 className="text-sm font-bold text-white tracking-wider uppercase mb-6 flex items-center gap-2">
                {t('pol.platform')}
              </h3>
              <ul className="space-y-4">
                {getVisibility('experts') && <li><Link to="/experts" className="text-slate-400 hover:text-white transition-colors flex items-center gap-1 group"><ChevronRight size={12} className="text-primary-500 opacity-0 group-hover:opacity-100 transition-opacity rtl:rotate-180" /> {t('nav.experts')}</Link></li>}
                {getVisibility('services') && <li><Link to="/services" className="text-slate-400 hover:text-white transition-colors flex items-center gap-1 group"><ChevronRight size={12} className="text-primary-500 opacity-0 group-hover:opacity-100 transition-opacity rtl:rotate-180" /> {t('nav.services')}</Link></li>}
                {getVisibility('pricing') && <li><Link to="/pricing" className="text-slate-400 hover:text-white transition-colors flex items-center gap-1 group"><ChevronRight size={12} className="text-primary-500 opacity-0 group-hover:opacity-100 transition-opacity rtl:rotate-180" /> {t('nav.pricing')}</Link></li>}
            ...
            {['COMPANY', 'RESOURCES', 'LEGAL'].map(section => {
              // Parse Pages
              const DEFAULT_PAGES = [
                { id: 'about', slug: '/about', title: 'About Us', section: 'COMPANY', isSystem: true, showPublic: true },
                { id: 'careers', slug: '/careers', title: 'Careers', section: 'COMPANY', isSystem: true, showPublic: true },
                { id: 'contact', slug: '/contact', title: 'Contact', section: 'COMPANY', isSystem: true, showPublic: true },
                { id: 'help-center', slug: '/qa', title: 'Help Center', section: 'RESOURCES', isSystem: true, showPublic: true },
                { id: 'zatca-guide', slug: '/compliance', title: 'ZATCA Guide', section: 'RESOURCES', isSystem: true, showPublic: true },
                { id: 'api-docs', slug: '#', title: 'API Documentation', section: 'RESOURCES', isSystem: false, showPublic: true },
                { id: 'privacy', slug: '/privacy', title: 'Privacy Policy', section: 'LEGAL', isSystem: true, showPublic: true },
                { id: 'terms', slug: '/terms', title: 'Terms of Service', section: 'LEGAL', isSystem: true, showPublic: true }
              ];

              let pages: any[] = [];
              try {
                pages = settings?.sitePages ? JSON.parse(settings.sitePages) : DEFAULT_PAGES;
              } catch (e) {
                pages = DEFAULT_PAGES;
              }

              const sectionPages = pages.filter((p: any) => p.section === section && p.showPublic);

              // Mapping for Section Titles (Basic map, can be enhanced with translation)
              const titles: Record<string, string> = {
                'COMPANY': t('pol.company'),
                'RESOURCES': t('pol.resources'),
                'LEGAL': t('pol.legal')
              };

              return (
                <div key={section}>
                  <h3 className="text-sm font-bold text-white tracking-wider uppercase mb-6">{titles[section] || section}</h3>
                  <ul className="space-y-4">
                    {sectionPages.map((page: any) => (
                      <li key={page.id}>
                        {page.slug.startsWith('http') || page.slug === '#' ? (
                          <a href={page.slug} className="text-slate-400 hover:text-white transition-colors">{page.title}</a>
                        ) : (
                          <Link to={page.slug} className="text-slate-400 hover:text-white transition-colors flex items-center gap-2">
                            {page.title}
                            {page.id === 'careers' && page.content?.hiringBadge && (
